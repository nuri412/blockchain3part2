...
describe("aITuSeModified (Modified)", function () {
  let Token;
  let token;
  let owner, addr1, addr2;

  beforeEach(async function () {
    Token = await ethers.getContractFactory("aITuSeModified");
    [owner, addr1, addr2] = await ethers.getSigners();
    token = await Token.deploy(2000); // Pass the initial value
  });
  ...
  describe("Deployment", function () {
    ...
    it("Should initialize with the provided initial value", async function () {
